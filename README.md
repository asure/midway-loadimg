...
|-------|-------|------------|
| **IMGTBL.ASM TROGMOUTH/MOUTHBACK** | Trog | Entries not in any source file (IMG/BDB/BDD/LOD) — ref may be from different source data |
| **BGNDTBL.ASM HDRS SAG values** | Trog | BBB handler encodes BDD images at different IRW positions vs LOADW |
| **BGNDPAL.ASM missing FACEPALS** | Trog | BBB handler missing `.long` palette alias entries (FACEPALS, LOGPALS) |
| **MK7 BGNDTBL.GLO ENDMARKER** | MK7 | ENDMARKER goes to current GLO (nba_spec.glo) not main IMGTBL.GLO |
| **BB7 OUTDOOR LEAF8 dedup** | BB7 | LEAF8 unused image; ref deduped to LEAF7 despite different SIZX. Inconsequential. |
| **NARC NARCMUGS SAG offset** | Narc | Our RLC encoder is correct; ref generated by buggy LOAD.EXE (`while run > 0`, stale 0xdd byte). See `RLC.md`. |
| **CARN total dedup cascade** | Carn | Triple-hash collision on TUNG3 cascades all subsequent SAGs (0/13). |

### Fixed in v0.96

- BB5: 7/7, BB6: 6/6 — PT0X reads `flags` (not `x1`) in IT.EXE mode; per-image `ite_pttbl`; heuristic extended to `n_seqscr=0`.
- BB5 PLYRDSP fixed (PT0X hardcode removed, x1→flags correction, per-image ite_pttbl).
- NARC NARCLOAF fixed (duplicate IMG records: LOADW uses LAST match).
- Trog IMGTBL.GLO, BGNDTBL.GLO pass (Motorola hex, tab separators, no blank lines).
- Trog: .TEXT skip for /OLD ASM/GLO files; .include BGNDTBL.GLO inserted; BLKS hdr_idx (`di*2`).
- Trog: IMGTBL.ASM headers, IMGPAL.ASM Motorola hex.
- /OLD: Motorola hex output (`>%X`), /H for Intel hex, POF>-based SAG+PAL combined/separate.
- Section_base_bit tracking, +META cache, PTTBL ptrdiff_t arithmetic.
- OUT_STRIDE minimum 1 for /OLD.

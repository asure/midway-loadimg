...
- **NARC: 20/21** — NARCLOAF fixed. The IMG file has duplicate `lfwk1` records: first uses `loafer` palette (palnum=3), last uses `fnklof` (palnum=7). LOADW uses LAST match; our code was breaking on FIRST for old-format IMGs due to version patching (0x634) triggering early-exit.
...
### Current Results: 14 pass, 7 fail
...
| **NARCMUGS TBL SAG offset** | Narc | Our RLC encoder is correct; ref TBL was generated by buggy LOAD.EXE (`while run > 0` loop, plus stale 0xdd byte after filestxt). See `RLC.md` §"The LOAD.EXE Encoder Bug" for details. |
| **CARN TUNG3 collision** | Carn | Triple-hash collision (722 diff pixels) |

Fixed since v0.91: BB5 (3/7→6/7), two ***> SAG, +META cache, /OLD IRW/BGNDTBL.GLO formatting, OUT_STRIDE for /OLD, Motorola hex output, IMGPAL.ASM, IMGTBL.GLO/BGNDTBL.GLO blank lines, .TEXT skip for /OLD ASM files, NARCLOAF duplicate IMG record (NARC 18→20/21).
